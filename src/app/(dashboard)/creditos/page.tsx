...
const TIPO_ACAO_LABEL: Record<string, string> = {
  ...
  sugerir_conteudo: 'Sugerir conteudo',
  gerar_documento: 'Gerar documento',
}

const PACOTES = [
  { creditos: 50,   preco: 'R$ 9,90',  destaque: false, descricao: 'Ideal para avaliar a plataforma.' },
  { creditos: 200,  preco: 'R$ 29,90', destaque: true,  descricao: 'Recomendado para uso regular.' },
  { creditos: 500,  preco: 'R$ 59,90', destaque: false, descricao: 'Para equipes com alto volume.' },
  { creditos: 1500, preco: 'R$ 149,90',destaque: false, descricao: 'Plano anual para grandes prefeituras.' },
]
...
export default async function CreditosPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900">Creditos de IA</h1>
        <p className="text-sm text-gray-500 mt-0.5">
          Gerencie seu saldo e acompanhe o consumo de recursos de inteligencia artificial.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">creditos disponiveis</p>
              </div>
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">creditos usados em IA</p>
              </div>
        ...
        <Card className="border-gray-200">
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Acoes realizadas</p>
      ...
      <Card className="border-gray-200">
        <CardHeader className="px-5 py-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Os pagamentos serao integrados em breve via Stripe e Mercado Pago.
          </p>
        </CardHeader>
        <CardContent className="p-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {PACOTES.map(pac => (
              ...
              >
                ...
                <p className="text-lg font-bold text-purple-700">{pac.preco}</p>
                <p className="text-xs text-gray-500 leading-relaxed">{pac.descricao}</p>
                <button
                  disabled
          ...
          <p className="text-xs text-gray-400 mt-3 text-center">
            Pagamentos via Stripe e Mercado Pago. Em desenvolvimento.
          </p>
        </CardContent>
      </Card>
          ...
          <CardContent className="p-0">
            {transacoes.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center px-4">
                ...
                <p className="text-xs text-gray-400 mt-1">Adquira creditos para comecar.</p>
              </div>
            ) : (
                    ...
                    <div key={t.id} className="flex items-center gap-3 px-5 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-gray-800 truncate">
                          {t.descricao ?? (isEntrada ? 'Recarga de creditos' : 'Consumo de IA')}
                        </p>
          ...
          <CardContent className="p-0">
            {acoes.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center px-4">
                ...
                <p className="text-sm text-gray-500">Nenhuma acao de IA registrada.</p>
                <p className="text-xs text-gray-400 mt-1">Use os botoes de IA nos documentos para gerar conteudo.</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-100 max-h-80 overflow-y-auto">
                {acoes.map((a: AcaoIA) => (
