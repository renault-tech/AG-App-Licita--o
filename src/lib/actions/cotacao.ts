...
export async function salvarCotacaoFornecedores(
  ...
  await (supabase.from('cotacoes_fornecedores') as any).delete().eq('cotacao_id', cotacaoId)
  ...
  if (fornecedores.length > 0) {
    ...
    await (supabase.from('cotacoes_fornecedores') as any).insert(fornecedoresInsert)
  }
