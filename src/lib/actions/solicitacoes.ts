...
async function notificarSetorCompras(
  ...
  const notifs = gestores.map((u: { id: string }) => ({
    usuario_id: u.id,
    titulo,
    mensagem,
    link,
...
export interface ItemSolicitacaoInput {
  ...
  especificacao_complementar?: string
  quantidade: number
...
export async function salvarSolicitacao(
  ...
  if (!ctx) return { success: false, error: 'Nao autorizado' }
  ...
  // Substitui itens
  await (supabase as any).from('solicitacoes_itens').delete().eq('solicitacao_id', id)
...
export async function enviarSolicitacao(
  ...
  if (!sol) return { success: false, error: 'Solicitacao nao encontrada' }
  ...
  await notificarSetorCompras(
    ...
    `${usuario.nome_completo} enviou uma solicitacao: ${sol.objeto}`,
    `/solicitacoes/${sol.id}`
  )
...
export async function recusarSolicitacao(
  ...
  await (supabase as any).from('notificacoes').insert({
    ...
    mensagem: `Sua solicitacao "${sol.objeto}" foi recusada. Motivo: ${motivo}`,
